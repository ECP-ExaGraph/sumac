...
NV_(0), NE_(0), maxOrder_(0), mass_(0)
{
    ...
    #pragma omp parallel
    {
        ...
        for(unsigned i = 0; i < 4; ++i)
            CudaCall(hipStreamCreate(&cuStreams[id][i]));
        ...
        CudaCall(hipDeviceSetCacheConfig(hipFuncCachePreferL1));
    ...
    for(int i = 0; i < NGPU; ++i)
    {
        ...
        {
           if(i != j)
                CudaCall(hipDeviceEnablePeerAccess(j, 0));
        }
...
GraphGPU::~GraphGPU()
{
    for(int g = 0; g < NGPU; ++g)
    {
        CudaSetDevice(g);
        for(unsigned i = 0; i < 4; ++i)
            CudaCall(hipStreamDestroy(cuStreams[g][i]));
...
)
{
    if(nv > 0)
    {
        ...
        CudaCall(hipMemGetInfo(&free_t,&total_t));
