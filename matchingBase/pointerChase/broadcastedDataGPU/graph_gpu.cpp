...
void GraphGPU::run_pointer_chase()
{
    ...
    #pragma omp parallel
    {
        ...
        while(batchCount != 0){
            ...
            for(int i=0;i<NGPU;i++){
                GraphElem offset = vertex_per_device_host_[i+1] - vertex_per_device_host_[i];
                ncclBroadcast((const void*)partners_+offset,(const void*)partners_+offset,ncclInt64,i,comm,)
            }
