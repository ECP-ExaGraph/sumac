...
template<const int BlockSize>
__global__
void match_pointer_chase_kernel
(
    GraphElem* indices_,
    ...
    GraphWeight* ws_,
    const GraphElem* vertex_per_device_,
    const int vertsPerWarp,
    const int device_id
)
{
    extern __shared__ GraphElem vqueue[];
    
    const GraphElem warp_id = threadIdx.x / 32;
    ...
    GraphElem* block_vqueue = &vqueue[vertsPerWarp*warp_id]; 
...
(
    ...
    GraphElem** partnersPtr_,
    GraphWeight* ws_,
    const GraphElem* vertex_per_batch_,
    const int device_id,
...
)
{
    ...
    CudaLaunch((match_pointer_chase_kernel<BLOCKDIM02><<<nblocks,threadCount,shmemSize>>>
    (indices_,edgeWeights_,edgeList_,mate_,matePtr_,partners_,partnersPtr_,ws_,vertex_per_batch_,vertsPerWarp,device_id)));
