...
NV_(0), NE_(0), maxOrder_(0), mass_(0)
{
    ...
    problematic_verts_host_ = new bool[NV_];
     ...
     #pragma omp parallel
    {
        ...
        for(unsigned i = 1; i < nbatches+1; i++){
            GraphElem newnv = vertex_per_batch_[id][i] - vertex_per_batch_[id][i-1];
            GraphElem newne = indicesHost_[vertex_per_batch_[id][i]-1] - indicesHost_[vertex_per_batch_[id][i-1]];
    ...
    CudaMalloc(mate_[id], sizeof(GraphElem) * maxnv);
    CudaMalloc(matePtr_[id], sizeof(GraphElem*) * NGPU);
    CudaMalloc(partners_[id], sizeof(GraphElem) * maxnv);
    CudaMalloc(partnersPtr_[id], sizeof(GraphElem*) * NGPU);
    CudaMalloc(ws_[id], sizeof(GraphWeight) * maxnv);


    CudaMalloc(problematic_verts_device_[id], sizeof(bool) * maxnv);

    CudaMemset(mate_[id],-1,sizeof(GraphElem) * maxnv);
    CudaMemset(partners_[id],-1,sizeof(GraphElem) * maxnv);
    ...
    CudaMemcpyUVA(vertex_per_batch_device_[id], vertex_per_batch_, sizeof(GraphElem) * nbatches+1);
...
GraphGPU::~GraphGPU()
{
    ...
    for(int g = 0; g < NGPU; ++g)
    {
        ...
        CudaFree(ws_[g]);
        CudaFree(problematic_verts_device_[g]);
...
void GraphGPU::run_pointer_chase(int batch_id)
{
    ...
    #pragma omp parallel
    {
        ...
        run_pointer_chase_cuda(indices_[id],edgeWeights_[id],edges_[id],mate_[id],matePtr_[id],partners_[id],partnersPtr_[id],ws_[id],
            vertex_per_batch_[id],id,threadCount,batch_id);
    }
...
void GraphGPU::move_batch_to_GPU(int batch_id){
    ...
    #pragma omp parallel
    {
        ...
        CudaMemcpyUVA(indices_[id], indicesHost_ + v0, sizeof(GraphElem) * nv);
        CudaMemcpyUVA(mate_[id], mate_host_ + v0, sizeof(GraphElem) * nv);
        CudaMemcpyUVA(partners_[id], partners_host_ + v0, sizeof(GraphElem) * nv);
        CudaMemcpyUVA(edges_[id], edgesHost_ + e0, sizeof(GraphElem) * ne);
        CudaMemcpyUVA(edgeWeights_[id], edgeWeightsHost_ + e0, sizeof(GraphElem) * ne);
