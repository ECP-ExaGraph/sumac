...
)
{
    ...
    for(int i=0;i<vertsPerWarp;i++){
        ...
        //if(threadIdx.x == 0 && g_warp_id == 0){
                //printf("%d - vertex_per_device_[1] = %ld\n",device_id,vertex_per_device_[1]);
                //printf("%d - currVert = %ld\n",device_id,currVert);
              //  printf("%d - matePtr_ = %ld\n",device_id,matePtr_[1][1]);
            //}
        if(currVert >= vertex_per_batch_device_[batch_id+1]){
            break;
        }

        if(mate_[currVertIdxDevice] != -1)
            continue;
        
        if(partners_[currVertIdxDevice] != -1){
            int targetGPU = find_target_GPU(partners_[currVertIdxDevice],vertex_per_device_);
            //if(threadIdx.x == 0 && g_warp_id == 0){
            //    printf("%d - targetGPU: %d\n",device_id,targetGPU);
            //}
            
            if(matePtr_[targetGPU][partners_[currVertIdxDevice]-vertex_per_device_[targetGPU]] == -1){
                continue;
            }
        }
        ...
        /*if(currVert == 0 || currVert == 1){
            printf("adj1 : %ld adj2 : %ld indices[1]: %ld - currVertIdx: %ld CurrVert: %ld\n",adj1,adj2,indices_[1],currVertIdx,currVert);
        }
        if(currVert == 1001){
            printf("adj1 : %ld adj2 : %ld indices[0]: %ld - currVertIdx: %ld\n",indices_[currVertIdx-1],indices_[currVertIdx],indices_[1],currVertIdx);
        }*/
        for(int j=(adj1-indices_[0])+lane_id;j<(adj2-indices_[0]);j+=WARPSIZE){ //Find heaviest edge among neighbors
            ...
            /*if(currVert == 1001){
                printf("Edge: %ld with weight %f\n",edge,weight);
            }*/
            if(edge == currVert)
                continue;
            if(weight>heaviest || (weight >= heaviest && edge>heaviestPartner)){
                int targetGPU = find_target_GPU(edge,vertex_per_device_);
        ...
        /*if(currVert == 0 || currVert == 1975){
            printf("heaviest partner %ld: %ld\n",currVert,heaviestPartner);
        }*/
        partnersPtr_[device_id][currVertIdxDevice] = heaviestPartner;
        /*if(lane_id == 0){
            printf("Vert: %ld partner set: %ld - %ld\n",currVert,partnersPtr_[device_id][currVertIdxDevice],partners_[currVertIdx]);
        }*/
...
)
{
    ...
    for(GraphElem currVert=vertex_per_batch_[batch_id]+tid;currVert<vertex_per_batch_[batch_id+1];currVert+=threadCount){
        ...
        if(partnersPtr_[targetGPU][currPartner-vertex_per_device_[targetGPU]] == currVert){
            mate_[currVert-vertex_per_device_[device_id]] = currPartner; 
...
)
{
    ...
    (indices_,edgeWeights_,edgeList_,mate_,matePtr_,partners_,partnersPtr_,vertex_per_batch_device_,vertex_per_device_,device_id,batch_id,vertsPerWarp)));
