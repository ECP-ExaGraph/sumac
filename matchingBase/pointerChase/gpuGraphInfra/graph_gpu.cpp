...
NV_(0), NE_(0), maxOrder_(0), mass_(0)
{
    ...
    #pragma omp parallel
    {
        ...
        CudaMalloc(mate_[id], sizeof(GraphElem) * nv);
        CudaMalloc(matePtr_[id], sizeof(GraphElem*) * NGPU);
        CudaMalloc(partners_[id], sizeof(GraphElem) * nv);
        CudaMalloc(partnersPtr_[id], sizeof(GraphElem*) * NGPU);
        CudaMalloc(ws_[id], sizeof(GraphWeight) * nv);

        CudaMemset(mate_[id],-1,sizeof(GraphElem) * nv);
        CudaMemset(partners_[id],-1,sizeof(GraphElem) * nv);
        CudaMemset(ws_[id],-1.0,sizeof(GraphWeight) * nv);
