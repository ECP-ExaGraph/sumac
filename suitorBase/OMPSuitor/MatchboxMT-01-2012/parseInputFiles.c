...
void parse_MatrixMarket_Sym_AsGraph(graph * G, char *fileName) {
    printf("Within parse_MatrixMarket_Sym_AsGraph(): Parsing a Matrix Market File as a nonbipartite graph...\n");
    int nthreads = 0;
#pragma omp parallel
    {
        nthreads = omp_get_num_threads();
    }
    printf("parse_MatrixMarket_Sym_AsGraph: Number of threads: %d\n ", nthreads);
    ...
    if ( isSymmetric==0 ) 	  {
        ...
        printf("Warning: .... Diagonal entries will be ignored. \n");
    } else {
        printf("Matrix is of type: Symmetric Real or Complex\n");
    }
...
#pragma omp parallel for
    ...
    long Si, Ti;
    double weight = 1;
    printf("Weights will be converted to positive real numbers.\n");
    edgeListTmp = (edge *) malloc(NE * sizeof(edge));
    while ( !feof(file)  ) {
        if (isPattern == 1)
            fscanf(file, "%ld %ld", &Si, &Ti);
